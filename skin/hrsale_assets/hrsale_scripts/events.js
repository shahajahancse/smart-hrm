$(document).ready(function() {
	...
	$("#xin-notice").submit(function(e){
		...
		$.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=1&add_type=notice&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					toastr.error(JSON.error);
					...
					xin_notice.api().ajax.reload(function(){ 
						toastr.success(JSON.result);
					}, true);
					$('input[name="csrf_hrsale"]').val(JSON.csrf_hash);
					$('.add-form').fadeOut('slow');
	...
	var xin_notice = $('#xin_notice').dataTable({
		"bDestroy": true,
		"ajax": {
			url : site_url+"events/notice_list/",
			type : 'GET'
		},
	});
	...
	// view data
		...
		var modal = $(this);
		$.ajax({
			url : site_url+"events/read_notice_record/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=view_event&notice_id='+notice_id,
			success: function (response) {
				if(response) {
					$("#ajax_modal_view").html(response);
				}
