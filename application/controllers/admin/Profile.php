	...
	public function output($Return=array()){
	  ...
	  public function index() {
		...
		$data = array(
			...
			'facebook_link' => isset($result[0]->facebook_link)? $result[0]->facebook_link:'',
			'twitter_link' => isset($result[0]->twitter_link)? $result[0]->twitter_link:'',
			'blogger_link' => isset($result[0]->blogger_link)? $result[0]->blogger_link:'',
			'linkdedin_link' => isset($result[0]->linkdedin_link)? $result[0]->linkdedin_link:'',
			'youtube_link' => isset($result[0]->youtube_link)? $result[0]->youtube_link:'',
			'google_plus_link' => isset($result[0]->google_plus_link)? $result[0]->google_plus_link:'',
			'instagram_link' => isset($result[0]->instagram_link)? $result[0]->instagram_link:'',
	...
	public function documents() {
        ...
        foreach($documents->result() as $r) {
			...
			if($r->document_file!='' && $r->document_file!='no file') {
			 $functions = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'download?type=document&filename='.$r->document_file.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"><i class="fa fa-download"></i></button></a></span>';
			 } else {
				 $functions ='';
			 }
