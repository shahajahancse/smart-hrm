...
class Accounting extends MY_Controller
{
    ...
    public function payment_out_page()
    {
        $session = $this->session->userdata('username');
        if (empty($session)) {
            redirect('admin/');
        }
        $data['title'] = "Payment Out";
        $data['breadcrumbs'] =  "Payment Out";
		$data['purposes'] = $this->db->get('xin_payment_out_purpose')->result();
		$this->db->select('i.*, p.title');
		$this->db->from('xin_payment_out_invoice AS i');
		$this->db->join('xin_payment_out_purpose AS p', 'p.id = i.purposes', 'left');
		$query = $this->db->get()->result();
		$data['table_data'] = $query;	
        $data['subview'] = $this->load->view("admin/accounting/payment_out_page", $data, true);
        $this->load->view('admin/layout/layout_main', $data); //page load
    }
    public function payout_form_submit()
    {
        $session = $this->session->userdata('username');
        if (empty($session)) {
            redirect('admin/');
		...
		$data= array(
            ...
            'Payment_Way' => $this->input->post('Payment_Way'),
            'Expense_Type' => $this->input->post('Expense_Type'),
            'date' => $this->input->post('date'),
            'note' => $this->input->post('note'),
		);

		$this->db->insert('xin_payment_out_invoice', $data);
		echo 'success';
    }
    public function get_purposes_amount()
    {
		$id=$this->input->get('id');
		$this->db->where('id',$id);
		$data = $this->db->get('xin_payment_out_purpose')->row();
