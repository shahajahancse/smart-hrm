...
class Xin_model extends CI_Model {
	...
	public function modify_salary($salary_month){
		...
		$this->db->select('s.basic_salary, s.late_deduct,s.m_pay_day,s.d_day, s.late_count, s.modify_salary,s.salary_month, e.first_name, e.last_name, e.user_id, ');
	...
	public function select_module_class($mClass,$mMethod) {
		...
		if($mClass=='dashboard') {
			...
			$arr['account'] = 'active';
			return $arr;
		} else if($mClass=='settings' && $mMethod=='account_setting_form') {
		...
		// attendance
			...
			$arr['attnd_open'] = 'active';
			return $arr;
		} else if($mClass=='attendance' && $mMethod=='employee_attendance') {
		...
		// lunch
			...
			return $arr;
		}  else if($mClass=='lunch' && $mMethod=='vendor_lunch_list') {
			$arr['lunch_open'] = 'active';
			$arr['vendor_list_active'] = 'active';
			return $arr;
		...
		// lunch
			...
			$arr['payrl_open'] = 'active';
			return $arr;
		} else if($mClass=='payroll' && $mMethod=='selary_emp') {
			$arr['selary_emp_active'] = 'active';
			$arr['payrl_open'] = 'active';
			return $arr;
		} else if($mClass=='payroll' && $mMethod=='employee_bonus') {
			$arr['emp_bonus_active'] = 'active';
			$arr['payrl_open'] = 'active';
			return $arr;
		...
		// inventory
			...
			$arr['per_rep_active'] = 'active';
			$arr['performance_open'] = 'active';
			return $arr;
		} else if($mClass=='performance_maingoals') {
			$arr['per_kpi_active'] = 'active';
			$arr['performance_open'] = 'active';
			return $arr;
		...
		// x// }
			...
			$arr['hr_all_quotes_active'] = 'active';
			$arr['hr_quote_manager_open'] = 'active';
			return $arr;
			...
			$arr['reports_open'] = 'active';
			return $arr;
		} else if($mClass=='reports'  && $mMethod=='inventory') {
			...
			$arr['reports_open'] = 'active';
			return $arr;
		}else if($mClass=='reports'  && $mMethod=='lunch_report_all') {
			$arr['reports_lunch_active'] = 'active';
			$arr['reports_open'] = 'active';
			return $arr;
		}else if($mClass=='reports'  && $mMethod=='employee_leave_report') {
			$arr['reports_leave_active'] = 'active';
			$arr['reports_open'] = 'active';
			return $arr;
	...
	public function get_notify_leave_applications_by_userid($user_id) {
		...
		if ($emp_data->is_emp_lead == 2) {
			...
			foreach ($extra_result as $row) {
				$extra[] = $row->user_id; // Store user_id values in $extra array
			}
			if (!empty($extra)) {
				$data=$this->db->select('xin_leave_applications.*')
					->from('xin_leave_applications')
					->join('xin_employees','xin_employees.user_id = xin_leave_applications.employee_id','LEFT')
					->where_in('xin_employees.status',[1,4,5])
	  			    ->where_in('xin_leave_applications.status',[1,4])
